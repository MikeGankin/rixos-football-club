...
Миксин оставляет в CSS обычный root-relative путь:

- `url("/hero-bg.jpg")`

Дальше путь переписывается автоматически:

- в dev: через `setupLocalCdnAssetRewrite()` из `src/lib/rewriteAssetsDev.js`
- на build: через `src/lib/rewriteAssetsBuild.mjs`

Так фоновые изображения работают одинаково и без склейки `var()` внутри `url(...)`.
